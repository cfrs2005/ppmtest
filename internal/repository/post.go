...
type PostRepository interface {
	...
	List(offset, limit int) ([]*models.Post, int64, error)
	GetByAuthorID(authorID uint, offset, limit int) ([]*models.Post, int64, error)
	GetByStatus(status string, offset, limit int) ([]*models.Post, int64, error)
...
func (r *postRepository) List(offset, limit int) ([]*models.Post, int64, error) {
...
func (r *postRepository) GetByAuthorID(authorID uint, offset, limit int) ([]*models.Post, int64, error) {
	var posts []*models.Post
	var total int64
	...
	result := query.Preload("Author").Order("created_at DESC").Offset(offset).Limit(limit).Find(&posts)
	if result.Error != nil {
		return nil, 0, result.Error
	}
...
func (r *postRepository) GetByStatus(status string, offset, limit int) ([]*models.Post, int64, error) {
	var posts []*models.Post
	var total int64
...
func (r *postRepository) Search(query string, offset, limit int) ([]*models.Post, int64, error) {
	...
	result := searchQuery.Preload("Author").Order("created_at DESC").Offset(offset).Limit(limit).Find(&posts)
	if result.Error != nil {
		return nil, 0, result.Error
	}
